...
export async function ensureJsonCleaningRegex(st_extension_settings, saveSettingsDebounced) {
    try {
        ...
        if (!st_extension_settings || typeof st_extension_settings !== 'object') {
            console.warn('[Dooms Tracker] Invalid extension_settings object, skipping JSON cleaning regex');
            return;
        }
        ...
        if (existingScript) {
            ...
            if (needsSave && typeof saveSettingsDebounced === 'function') {
                ...
                console.log('[Dooms Tracker] ✅ Updated JSON cleaning regex to v3.2.3 settings.');
            } else {
                console.log('[Dooms Tracker] JSON Cleaning Regex is up to date.');
            }
            return;
        }
        ...
        console.log('[Dooms Tracker] JSON Cleaning Regex created and activated.');
        // Save the changes
        if (typeof saveSettingsDebounced === 'function') {
            saveSettingsDebounced();
        } else {
            console.warn('[Dooms Tracker] saveSettingsDebounced is not a function, cannot save JSON cleaning regex');
        }
    } catch (error) {
        console.error('[Dooms Tracker] JSON Cleaning Regex failed to properly initialize!');
...
export function removeJsonCleaningRegex(st_extension_settings, saveSettingsDebounced) {
    try {
        ...
        console.error('[Dooms Tracker] Failed to remove JSON cleaning regex:', error);
    }
}
