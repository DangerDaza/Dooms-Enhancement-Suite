...
export async function ensureHtmlCleaningRegex(st_extension_settings, saveSettingsDebounced) {
    try {
        ...
        if (!st_extension_settings || typeof st_extension_settings !== 'object') {
            console.warn('[Dooms Tracker] Invalid extension_settings object, skipping HTML regex import');
            return;
        }
        ...
        if (!Array.isArray(existingScripts)) {
            console.warn('[Dooms Tracker] extension_settings.regex is not an array, resetting to empty array');
        ...
        if (typeof saveSettingsDebounced === 'function') {
            ...
            console.warn('[Dooms Tracker] saveSettingsDebounced is not a function, cannot save HTML regex');
        }
    } catch (error) {
        console.error('[Dooms Tracker] Failed to import HTML cleaning regex:', error);
...
export async function ensureTrackerCleaningRegex(st_extension_settings, saveSettingsDebounced) {
    try {
        // Validate extension settings structure
        if (!st_extension_settings || typeof st_extension_settings !== 'object') {
            console.warn('[Dooms Tracker] Invalid extension_settings object, skipping tracker cleaning regex import');
            return;
        }
        ...
        const regexScript = {
            id: uuidv4(),
            scriptName: scriptName,
            findRegex: '/```json\\s*\\n\\{[\\s\\S]*?(?:\"quests\"|\"infoBox\"|\"characters\"|\"userStats\")[\\s\\S]*?\\}\\s*\\n```/gm',
            replaceString: '',
            trimStrings: [],
            placement: [2], // 2 = Input (affects outgoing prompt)
            disabled: false,
        ...
        if (typeof saveSettingsDebounced === 'function') {
            ...
            console.warn('[Dooms Tracker] saveSettingsDebounced is not a function, cannot save tracker cleaning regex');
        }
    } catch (error) {
        console.error('[Dooms Tracker] Failed to import tracker cleaning regex:', error);
        console.error('[Dooms Tracker] Error details:', error.message, error.stack);
        // Don't throw - this is a nice-to-have feature
    }
}
