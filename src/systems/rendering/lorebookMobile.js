/**
 * Lorebook Mobile Renderer — main branch single-panel layout for mobile devices.
 * This is the proven production renderer from the main branch, used when viewport ≤ 600px.
 */
...
export function initMobileLorebookEventDelegation() {
    ...
    // Shared handlers (close, backdrop, escape, campaign header, campaign
    // toggle, book toggle, entry toggle, delete, import, export, icon picker,
    // global settings, state selects, tab-add, section dividers, etc.) are
    // registered by the v2 module's initLorebookEventDelegation().
    // Those use the same selectors and call renderLorebook() which branches
    // to renderMobileLorebook() on mobile. DO NOT duplicate them here.
    // ─────────────────────────────────────────────────────────────────────────
    ...
    // Book toggle, campaign toggle, entry toggle — handled by v2's initLorebookEventDelegation()

    // ── Tab switching (mobile-only UI) ────────────────────────────────────
    ...
    // All handlers below (section dividers, entry/book delete, add entry,
    // new book, import, export, spine edit, state selects, global settings,
    // icon picker, field editing) use selectors shared with v2 — handled there.

    // ── Mobile entry-body field editing (uses .rpg-lb-entry-body, not .rpg-lb-editor) ──
    ...
    // ── Mobile add-entry (uses .rpg-lb-lore-entries container) ──────────────
    ...
    // ── Spine edit (open in ST's native WI editor) ──────────────────────────
