...
import { generateTrackerInstructions, generateTrackerExample } from './promptBuilder.js';
...
export async function buildEncounterInitPrompt() {
    ...
    try {
        ...
        console.warn('[Dooms Tracker] Failed to get world info from getWorldInfoPrompt:', e);
    }
    ...
    if (!worldInfoAdded) {
        console.warn('[Dooms Tracker] ⚠️ No world information available');
    ...
    // NOTE: userStats/inventory/classicStats context removed — system archived to src/archived/
    // Build scene context info
    let sceneContextInfo = '';
    // Add quests context if available
    if (committedTrackerData.quests) {
        sceneContextInfo += `Current Quests:\n${committedTrackerData.quests}\n\n`;
    }
    ...
    initInstruction += `Populate the party data based on the scene context. For ${userName}'s attacks array, include abilities appropriate to the character. Set HP based on reasonable defaults for the character.\n\n`;
...
export async function buildCombatActionPrompt(action, combatStats) {
    ...
    try {
        ...
        console.warn('[Dooms Tracker] Failed to get world info for combat action:', e);
    }
    if (!worldInfoAdded && context.activatedWorldInfo && Array.isArray(context.activatedWorldInfo) && context.activatedWorldInfo.length > 0) {
        context.activatedWorldInfo.forEach((entry) => {
            if (entry && entry.content) {
    ...
    if (context.name1) {
        ...
        // NOTE: Classic stats/attributes removed — userStats system archived
        systemMessage += `</persona>\n\n`;
    }
    messages.push({
        role: 'system',
...
export async function buildCombatSummaryPrompt(combatLog, result) {
    ...
    try {
        ...
        console.warn('[Dooms Tracker] Failed to get world info for summary:', e);
    }
    // Fallback to activatedWorldInfo
    if (!worldInfoAdded && context.activatedWorldInfo && Array.isArray(context.activatedWorldInfo) && context.activatedWorldInfo.length > 0) {
        context.activatedWorldInfo.forEach((entry) => {
    ...
    if (extensionSettings.generationMode === 'together' && (extensionSettings.showQuests || extensionSettings.showInfoBox || extensionSettings.showCharacterThoughts)) {
        summaryMessage += `\n--- TRACKER UPDATE ---\n\n`;
        summaryMessage += `After the [FIGHT CONCLUDED] summary, update the RPG trackers to reflect the state AFTER the combat encounter. `;
        summaryMessage += `Account for any emotional state changes, or other consequences of the battle.\n\n`;
        // Include pre-combat tracker state if available
        if (committedTrackerData.quests || committedTrackerData.infoBox || committedTrackerData.characterThoughts) {
            summaryMessage += `Pre-combat tracker state:\n`;
            summaryMessage += `<previous>\n`;
            if (committedTrackerData.quests) {
                const questsJSON = typeof committedTrackerData.quests === 'object'
                    ? JSON.stringify(committedTrackerData.quests, null, 2)
                    : committedTrackerData.quests;
                summaryMessage += questsJSON + '\n';
            }
        ...
        const trackerInstructions = generateTrackerInstructions(false, false);
...
export function parseEncounterJSON(response) {
    try {
        ...
        if (!response || typeof response !== 'string') {
            console.error('[Dooms Tracker] parseEncounterJSON received non-string input:', typeof response);
            return null;
        }
        ...
        if (firstBrace !== -1 && lastBrace !== -1) {
            ...
            console.error('[Dooms Tracker] No JSON object found in response');
            return null;
        }
        ...
        try {
            ...
            console.warn('[Dooms Tracker] Initial parse failed, attempting JSON repair...');
        ...
        console.error('[Dooms Tracker] Failed to parse encounter JSON:', error);
        console.error('[Dooms Tracker] Response was:', response);
        return null;
    }
}
