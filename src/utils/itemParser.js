...
export function parseItems(itemString) {
    ...
    for (let i = 0; i < processed.length; i++) {
        ...
        if (char === '(') {
            ...
            if (parenDepth < 0) {
                console.warn('[Dooms Tracker] Unmatched closing parenthesis in item parsing');
                parenDepth = 0;
            }
            currentItem += char;
            ...
            if (isDecimalComma) {
                ...
                if (cleaned) {
                    ...
                    if (items.length >= MAX_ITEMS_PER_SECTION) {
                        console.warn(`[Dooms Tracker] Reached max items limit (${MAX_ITEMS_PER_SECTION}), truncating list`);
                        return items;
                    }
                }
    ...
    if (parenDepth > 0) {
        console.warn('[Dooms Tracker] Unmatched opening parenthesis in item parsing');
    }
    return items;
}
/**
