...
- [ ] Ingestion duplicate protection: tracker name 'X (anything)' or 'The X'/'X' variants of an existing card fold into it silently + alias auto-recorded; fuzzy-similar new names raise ONE yes/no dialog per pair (DES-themed, follows selected theme incl. on first message before settings ever opened) — Yes adds alias + scrubs the duplicate card/color/avatar and repaints, No dismisses permanently, Escape/backdrop = ask again later; CHAT BUBBLES WAIT for the open dialog before attributing dialogue (answer Yes → bubble speaker correct, no stale attribution); exact existing card names untouched; no dialogs on chat load
