...
- [ ] **Open Prompts Editor on a fresh profile, change nothing, hit Save — `customTrackerPrompt` must stay empty.** Then add a tracker field and confirm it still reaches the AI. (Regression: the no-op Save used to bank a frozen override that detached the prompt from every tracker setting.) Same check after editing ONLY the Tracker Instructions box
- [ ] "Restore All Prompts" clears the Tracker Prompt override too: the full-prompt box refills with the generated text, the "edited" badge clears, and a following Save doesn't re-bank it
- [ ] Duplicate-character dialog on a real phone: fully on screen with the heading visible (not just the buttons), stays put while browser chrome slides in/out and with the keyboard open, scrolls inside itself rather than overflowing on a short landscape screen, and both buttons are tappable at 360px wide
- [ ] Scene Tracker set to **bottom ticker**: two turns with identical scene data must NOT rebuild it (the scroll animation shouldn't restart); changing location/time still updates it
...
- [ ] **Workshop stage layout**: the portrait fills the left column edge to edge with name/relationship over a bottom gradient; the six section tabs (Identity … Knives, Voice) run horizontally (icons only at ≤1080px, scrollable strip on a phone) and every existing control keeps working — Identity chips/aliases, Appearance upload/clear/Write prompt/Render portrait/colour palette, Expressions lazy-load (sprites are name-scoped, the tab does NOT reload on a version switch), Injection combobox/prompt/attach/banish/clear-injects, Knives add/delete/re-arm/Generate, footer Delete/Export/Cancel/Copy to Users/Eject/Inject/Save; user-character mode (data-mode="user") hides the version strip and campaign badge; all ten DES themes keep the stage legible
- [ ] **Workshop versions**: the strip appears only once a campaign exists (Base + one tile per campaign version + "+"); opening a card lands on the version the chat is using (badged LIVE); the "+" tile lists campaigns without a version and clones the version on the stage; a version tile switches with a crossfade + slide (instant under performance mode / prefers-reduced-motion), keeps the current tab and half-typed knife/alias text, and asks before discarding unsaved versioned edits (colour and alias edits carry across); Save on a non-live version writes that version's saved copy (check the campaign's portrait/description after activating it); Render portrait is disabled on a non-live version with an explanatory tooltip; the "×" on a campaign tile removes that version (character falls back to Base there) and keeps a portrait file Base still uses; Delete character removes every version and names the count; a campaign switch while the card is open reloads the live version (asking first when there are unsaved edits); Arrow/Home/End keys move between tiles, Enter/Space activates
- [ ] **Roster**: "Viewing <campaign>" chip above the grid once campaigns exist (hidden in Users mode), tiles with extra versions show a "⧉ N" badge that never overlaps the relationship / colour dot / pin / ACTIVE badges; deleting from the Roster removes every version and only unreferenced portrait files

## Voices (DES per-character TTS — docs/google-tts-voices-plan.md)
- [ ] **Voices off costs nothing**: only `voiceBoot.js` is in the startup graph (`node tools/lazy-graph-test.mjs`); the bubble / thought / reasoning bullhorns use SillyTavern's `/speak` as before, and a `|` or `{{` in the text no longer breaks the command
- [ ] **Settings → Voices**: master toggle, auto-read, Narrator voice (30 standard voices, ▶ preview), model (3.8 Flash-Lite / Flash), speed, "read my own messages", request limit; the status line names the route and the model actually used
- [ ] **SillyTavern's auto-read is paused** while DES voices are on and resumes when they're off; its checkbox still records changes and its saved value on disk never changes (reload SillyTavern and check the box)
- [ ] **Workshop Voice tab**: 30 standard voices with preview, pick + Save stores the voice, "Use Narrator (no voice)" clears it; the voice is per campaign version and "+" gives the new version Base's voice; personas get the tab without versions; shared-colour warning shows when two characters share a colour
- [ ] **Scene rule**: a voice plays only while the character is on the Present Characters panel for that message; narration, unattributed lines, absent/removed/banned characters and characters with no voice use the Narrator (the browser console's `[DES Voices] job` line gives the reason per line)
- [ ] **Bullhorns** (bubble "read from here", DES message button, thought, reasoning) use DES voices while voices are on; clicking the one that's playing stops it; the highlight clears when the line ends
...
- [ ] **Clone a voice** (needs the DES key): Continue blocked until the permission box is ticked; sample must be 10–30 s and audible (upload or record; recording needs HTTPS/localhost, upload always offered); consent statement shown verbatim for the chosen locale (30 locales); both clips sent as 24 kHz mono 16-bit WAV; a rejected consent shows Google's reason and Re-record buttons; recordings are dropped after creating, on Start over, and when the Workshop closes (never in settings or browser storage); Use this voice + Save stores it with a same-gender fallback; Settings lists it as Cloned, with no Recreate (record again to renew)
- [ ] Creating (or recreating) a designed voice reads its description aloud in the new voice; Read description replays it; Google's sample button still works
...
- [ ] **Active campaign** (desktop + mobile): each campaign header has a Set-active control and the active one is badged ACTIVE; activating turns that campaign's books on and the previous campaign's books off (a book switched on by hand outside any campaign stays on; a book flagged **global** via the globe on its row stays on across switches); clicking the active campaign again deactivates it; a second click while a switch is still running is ignored rather than interleaved; the footer names the active campaign; deleting the active campaign deactivates it first; renaming/deleting/moving a book keeps the active campaign's books on and the ledger straight (run `node tools/campaign-profiles-test.mjs`)
- [ ] Turning the Lore Library feature OFF while a campaign is active deactivates the campaign (character stores swap back to Base)
- [ ] **Auto-link by name** (Settings → Lore Library toggle, on by default): a book named exactly like a chat cast member switches on when they join the cast and off when they leave; a book switched on by hand is never switched off; a book the active campaign keeps on (or a global) stays on when its character leaves; a campaign switch never turns off a book whose character is still in the cast; no chat cast = nothing happens (run `node tools/campaign-switch-test.mjs`)
- [ ] With no campaign active, every identity store and every Workshop/Roster/portrait-bar behaviour is byte-identical to 2.5.0 (`campaignBaseShadow` stays `{}`)
...
- [ ] Character Workshop → Identity → **Relationship to player** carousel: chips are generated from Settings → Workshop (add one there, reopen a card, it's on the end); the row stays ONE line at a fixed height however many exist; arrows scroll and disable at each end, and go invisible-but-space-holding when the list already fits; opening a card lands on the relationship already set (no visible slide — the jump must be instant, only arrow presses animate); a relationship set on a card but deleted from the list still shows and stays selected; clicking the selected chip still clears it; user-mode cards show pronouns here instead and are unaffected
