...
- [ ] Tracker prompt no longer requests details/relationship/stats for characters, but DOES still request name, emoji, color and thoughts — PCP cards, bubble speaker attribution and thought dropdowns all still work (run `node tools/tracker-prompt-test.mjs`)
