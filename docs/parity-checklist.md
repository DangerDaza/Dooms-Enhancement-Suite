...
- [ ] Ingestion duplicate protection: tracker name 'X (anything)' or 'The X'/'X' variants of an existing card fold into it silently + alias auto-recorded; fuzzy-similar new names toast once (click opens Workshop) and are NOT merged; exact existing card names untouched; no toast on chat load
