...
- [ ] **Connection**: Settings → Voices → Connection lists SillyTavern connection profiles (non-Google ones greyed out); a Google profile's saved key is active only while a voice request is in flight and SillyTavern's previous key is active again afterwards (check API Connections); a chat sent mid-read goes out on the chat key; a deleted profile gives one clear toast and sends nothing
