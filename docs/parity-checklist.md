...
- [ ] Ingestion duplicate protection: tracker name 'X (anything)' or 'The X'/'X' variants of an existing card fold into it silently + alias auto-recorded; fuzzy-similar new names raise ONE yes/no popup per pair — Yes adds alias + scrubs the duplicate card/color/avatar and repaints, No dismisses permanently; exact existing card names untouched; no popups on chat load
